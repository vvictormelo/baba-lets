...
export async function GET(req: NextRequest) {
  ...
  // Rodadas em que participou (confirmou presença)
  ...
  if (!participations?.length) {
    return NextResponse.json({ active_round_id: activeRoundId, history: [] }, { headers: { 'Cache-Control': 'no-store' } })
  }

  const roundIds = participations.map(p => p.round_id)

  // Pote atribuído em cada rodada
  const { data: pots } = await supabase
    .from('round_pots')
    .select('round_id, pote')
    .eq('player_id', Number(player_id))
    .in('round_id', roundIds)

  // Time sorteado em cada rodada
  const { data: teams } = await supabase
    .from('round_teams')
    .select('round_id, team, pote')
    .eq('player_id', Number(player_id))
    .in('round_id', roundIds)

  const potMap = Object.fromEntries((pots || []).map(p => [p.round_id, p.pote]))
  const teamMap = Object.fromEntries((teams || []).map(t => [t.round_id, { team: t.team, pote: t.pote }]))
  ...
  const history = participations
    .map(p => {
      ...
      return {
        ...
        team: teamMap[p.round_id]?.team ?? null,
  ...
  return NextResponse.json(
    { active_round_id: activeRoundId, history },
